...
Ensure you have a Redis server running locally or via Docker and Ollama accessible on its default port.
...
- `tasks.py`: Background job tasks run by the RQ worker.
...
- `uploads/`: Temporary directory for local file storage during processing.
...
- [ ] **Docker Setup:** Update and verify the `Dockerfile` and `docker-compose.yml` to properly orchestrate the API, RQ worker, Redis, and Database with optimal configurations.
- [ ] **S3 Object Storage (MinIO):** Replace the local `uploads/` dir with real object storage by integrating an S3-compatible service like MinIO for handling document uploads reliably across distributed workers.
